 ...
 * - 5th byte is a flag for if an ACK packet must be returned.
 * - 6th byte is the packet incrementer. This is incremented for every new byte addressed to a specific reciever. Used for counting lost packets.
 * - 7 to (6+n)th byte is the raw data
 * - (7+n)th byte is the crc byte to check if data is valid
 * - (8+n) is the endbyte marker
 */
...
public:
    ...
    /**
     * Makes packet require ACK return for successfull send.
     * 
     * Give false to disable ack request.
     *
     * @param values requestAck.
     * @return void.
     */
    void requestACK(bool requestAck = true) {_ackRequest = requestACK;}
...
public:
    ...
    uint32_t packDataIntoBuffer(uint8_t* dataBytes, const uint32_t &dataByteSize) {
        ...
        dataBytes[4] = _ackRequest;

        dataBytes[5] = _dataID;
        dataBytes[6] = _dataSize;
        
        for (uint32_t i = 0; i < sizeof(_packetBytes); i++) dataBytes[7+i] = _packetBytes[i]; //Move data from buffer into output

        dataBytes[_dataSize + 7] = _crc = calculateCRC();

        dataBytes[_dataSize + 8] = KRAFTPACKET_MARKER_ENDBYTE;

        return _dataSize + 8 + 1;
    ...
    bool unpackDataFromBuffer(const uint8_t* dataBytes, const uint32_t &dataByteSize) {
        ...
        _ackRequest = dataBytes[4];

        _dataID = dataBytes[5];
        _dataSize = dataBytes[6];

        if (_dataSize > dataByteSize) return false;
        
        for (uint32_t i = 0; i < _dataSize; i++) _packetBytes[i] = dataBytes[7+i]; //Move data from buffer into output

        if (dataBytes[_dataSize + 8] != KRAFTPACKET_MARKER_ENDBYTE) return false;

        _crc = calculateCRC();

        if (dataBytes[_dataSize + 7] != _crc) return false;
