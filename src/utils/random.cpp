#include "../../include/KraftKontrol/utils/random.h"


#include "math.h"


uint16_t randGen(uint16_t max = UINT16_MAX, uint32_t seed = 0) {
...
float randGenFloat() {
    return ( (float)(randGen()) + 1. )/( (float)(UINT16_MAX) + 1. );
}


float randNorm(float sigma = 1, float mi = 0) {
